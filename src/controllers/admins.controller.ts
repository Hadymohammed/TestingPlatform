...
const getById=async (req:Request,res:Response):Promise<void>=>{
    try{
        ...
        if(id==null){res.status(400).send('admin_id is missing');return;}
...
const getByNational=async (req:Request,res:Response):Promise<void>=>{
    try{
        ...
        if(nid==null){res.status(400).send('national_id is missing');return;}
...
const register=async (req:Request,res:Response):Promise<void>=>{
    try{ 
        ...
        if(await adminEntity.uniqueUsername(admin.username) == false)uniqueError.push('username');
        if(await adminEntity.uniqueNational(admin.national_id) == false)uniqueError.push('national_id');
